// avaxGAS (ERC20)
// The 5th Token in the Gas Station ecosystem.
...
contract avaxGAS is ERC20, Ownable {
    ...
    avaxGASDividendTracker public dividendTracker;
    ...
    // store addresses that a autoAVAX market maker pairs. Any transfer *to* these addresses
    ...
    event ProcessedDividendTracker(
    	...
    	bool indexed autoAVAX,
    	uint256 gas,
    	address indexed processor
    );

    constructor() public ERC20("avaxGAS", "avaxGAS") {

    	dividendTracker = new avaxGASDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "avaxGAS: The dividend tracker already has that address");

        avaxGASDividendTracker newDividendTracker = avaxGASDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "avaxGAS: The new dividend tracker must be owned by the avaxGAS token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "avaxGAS: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded, bool excludedFromTrades) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded || _isExcludedFromFeesDuringTrades[account] != excludedFromTrades, "avaxGAS: Account is already the value of 'excluded' and 'excludedFromTrades'");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "avaxGAS: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "avaxGAS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "avaxGAS: Cannot update gasForProcessing to same value");
...
contract avaxGASDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    event Claim(address indexed account, uint256 amount, bool indexed autoAVAX);

    constructor() public DividendPayingToken("avaxGAS_Dividen_Tracker", "avaxGAS_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "avaxGAS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "avaxGAS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main avaxGAS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "avaxGAS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "avaxGAS_Dividend_Tracker: Cannot update claimWait to same value");
    ...
    function processAccount(address payable account, bool autoAVAX) public onlyOwner returns (bool) {
        uint256 amount = _withdrawDividendOfUser(account);

    	if(amount > 0) {
    		lastClaimTimes[account] = block.timestamp;
            emit Claim(account, amount, autoAVAX);
    		return true;
    	}
